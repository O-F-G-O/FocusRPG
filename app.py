...
st.markdown("""
    ...
    .achievement-card { background: white; padding: 20px; border-radius: 12px; border: 2px solid #FFD700; text-align: center; margin-bottom: 15px; }
    </style>
...
with c_b1: draw_bar("EXPÉRIENCE", (xp_in/xp_req)*100, "xp-fill")
...
# DASHBOARD
# ==============================================================================
if 'current_page' not in st.session_state or st.session_state['current_page'] == "Dashboard":
    ...
    with col_r:
        ...
        with cs2:
            ...
            if 'gym_current_prog' in st.session_state and st.session_state['gym_current_prog']:
                n, d = st.session_state['gym_current_prog']; st.info(f"**{n}**")
                for l in d.split('\n'): st.write(f"- {l}")
...
# ==============================================================================
                    ...
                    for i, row in enumerate(df_c.iterrows()):
                        with cols[i%2]:
                            if st.button(f"🔥 {row[1]['Chapitre']}", key=f"a{b['Nom']}{i}"):
                                # Logic Attaque
                                ws_t = get_db().worksheet("Boss_Tasks"); cell = ws_t.find(row[1]['Chapitre'])
                                if cell: ws_t.delete_rows(cell.row)
                                ws_b = get_db().worksheet("Bosses"); b_cell = ws_b.find(b['Nom'])
                                new_pv = max(0, float(ws_b.cell(b_cell.row, 4).value) - dmg)
                                ws_b.update_cell(b_cell.row, 4, new_pv)
                                save_xp(10, "Combat Boss", b['Nom']); st.rerun()
                else:
    ...
    if df_raw.empty: st.write("Aucun exploit enregistré.")
    else:
